...
describe('CRDTEntity', () => {
    ...
    it('can apply a set operation to a singleton field', () => {
        ...
        assert.deepEqual(entity.getParticleView(), { singletons: { name: value }, collections: { tags: new Set() } });
    });
    ...
    it('can apply operations to multiple fields', () => {
        const singletons = {
            name: new CRDTSingleton(),
            age: new CRDTSingleton()
        };
        const collections = {
            tags: new CRDTCollection(),
            favoriteNumbers: new CRDTCollection()
        };
        const entity = new CRDTEntity(singletons, collections);
    ...
    it('fails when singleton operations are provided to collection fields', () => {
        ...
        assert.throws(() => entity.applyOperation({ type: EntityOpTypes.Set, field: 'things', value: { id: 'foo' }, actor: 'me', clock: { 'me': 1 } }), `Can't apply Set operation to collection field`);
        assert.throws(() => entity.applyOperation({ type: EntityOpTypes.Clear, field: 'things', actor: 'me', clock: { 'me': 1 } }), `Can't apply Clear operation to collection field`);
    });
