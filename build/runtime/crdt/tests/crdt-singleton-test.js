...
describe('CRDTSingleton', () => {
    ...
    it('can clear values', () => {
        ...
        singleton.applyOperation({ type: SingletonOpTypes.Clear, actor: 'A', clock: new Map([['A', 1]]) });
        assert.equal(singleton.getParticleView(), null);
    });
    it('can add and clear from multiple actors', () => {
        ...
        assert.deepEqual(singleton.getData().values, new Map([['1', new Map([['A', 1]])]]));
        assert.equal(singleton.getParticleView(), '1');
        ...
        assert.deepEqual(singleton.getData().values, new Map([['1', new Map([['A', 1]])], ['2', new Map([['B', 1]])]]));
        assert.equal(singleton.getParticleView(), '1');
        ...
        assert.deepEqual(singleton.getData().values, new Map([['2', new Map([['A', 1], ['B', 2]])]]));
        assert.equal(singleton.getParticleView(), '2');
        singleton.applyOperation({
            type: SingletonOpTypes.Clear,
            actor: 'A',
    ...
    it('can merge two singletons', () => {
        ...
        const singletonB = new CRDTSingleton();
        ...
        if (modelChange.changeType === ChangeType.Model) {
            assert.deepEqual(modelChange.modelPostChange, { values: newValues, version: newVersion });
        }
        else {
