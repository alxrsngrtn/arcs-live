...
import { ChangeType, isEmptyChange } from '../crdt.js';
...
function removeOp(id, actor, clock) {
...
describe('CRDTCollection', () => {
    ...
    it('rejects values with missing IDs', () => {
        const set = new CRDTCollection();
        const valuesWithMissingIDs = [
            {},
            { id: null },
            { id: '' },
        ];
        valuesWithMissingIDs.forEach(value => {
            assert.throws(() => set.applyOperation({ type: CollectionOpTypes.Add, added: value, actor: 'a', clock: { a: 1 } }), 'CRDT value must have an ID.');
            assert.throws(() => set.applyOperation({ type: CollectionOpTypes.Remove, removed: value, actor: 'a', clock: { a: 1 } }), 'CRDT value must have an ID.');
        });
    });
    ...
    describe('fast-forward operations', () => {
        ...
        it('can add elements', () => {
            ...
            assert.deepEqual(set.getData(), {
                values: {
                    'one': { value: { id: 'one' }, version: { a: 1, b: 7 } },
                    'three': { value: { id: 'three' }, version: { a: 2, b: 1 } },
                    'four': { value: { id: 'four' }, version: { a: 1, b: 9 } },
                },
                version: { a: 2, b: 9 },
            });
        });
