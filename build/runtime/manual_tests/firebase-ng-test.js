...
async function resetStorageKeyForTesting(key) {
...
describe('firebase-ng-driver', function () {
    ...
    it('can write to a new location', async () => {
        ...
        const driver = new FirebaseDriver(storageKey, Exists.ShouldCreate);
        await driver.init();
        driver.registerReceiver((model, version) => { assert.fail(); });
        await driver.send(24, 1);
        const output = new FirebaseDriver(storageKey, Exists.ShouldExist);
        await output.init();
    ...
    it('will reject one of two synchronous writes', async () => {
        const storageKey = new FirebaseStorageKey(testProject, testDomain, testKey, 'foo');
        await resetStorageKeyForTesting(storageKey);
        const driver1 = new FirebaseDriver(storageKey, Exists.ShouldCreate);
        await driver1.init();
        const driver2 = new FirebaseDriver(storageKey, Exists.ShouldExist);
        await driver2.init();
        const receivedData = new Promise(async (resolve, reject) => {
            driver2.registerReceiver((model, version) => {
...
//# sourceMappingURL=firebase-ng-test.js.map