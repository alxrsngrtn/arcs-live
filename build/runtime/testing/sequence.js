...
(function (ExpectedResponse) {
    ExpectedResponse[ExpectedResponse["Void"] = 0] = "Void";
    ExpectedResponse[ExpectedResponse["Defer"] = 1] = "Defer";
    ExpectedResponse[ExpectedResponse["Constant"] = 2] = "Constant";
})(ExpectedResponse || (ExpectedResponse = {}));
export var SequenceOutput;
(function (SequenceOutput) {
    SequenceOutput[SequenceOutput["Replace"] = 0] = "Replace";
    SequenceOutput[SequenceOutput["Register"] = 1] = "Register";
...
export class SequenceTest {
     ...
     */
    setTestConstructor(prepareFunction) {
     ...
     */
    registerInput(name, asyncCount, response) {
     ...
     */
    registerOutput(name, response, behavior, variable = null) {
     ...
     */
    registerVariable(initialValue, initializerIsFunction = false) {
    ...
    setChanges(id, changes) {
        this.changes[id] = changes.map(({ input, inputFn, output, variable }) => ({ input, inputFn, output, variable, id }));
    }
    /**
     ...
     * etc..
     */
    *interleavings_raw(length, amounts) {
        const currentChoice = [];
    ...
    *choosePoint(previous, interleaving, interleavingDelays) {
        ...
        for (let i = interleaving.length; i > previous; i--) {
            if (interleavingDelays[i - 1] > 0) {
                ...
                nextInterleavingDelays = nextInterleavingDelays.concat(this.computeInterleavingDelays(interleavingDelays[i - 1] - 1, interleaving.slice(i)));
    ...
    // delays.
    computeInterleavingDelays(max, interleaving) {
        const result = [];
    ...
    *interleavings() {
        ...
        while (true) {
            ...
            const newInterleaving = interleaving.map((index) => {
