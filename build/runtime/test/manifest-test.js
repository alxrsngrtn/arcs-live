...
${particleStr1}
  ...
  modality dom`;
    ...
    `);
        ...
        const recipe = manifest.recipes[0];
        assert.lengthOf(recipe.slots, 2);
        assert.equal(checkDefined(recipe.particles.find(p => p.name === 'ParticleB')).consumedSlotConnections['slotB1'].providedSlots['slotB2'], checkDefined(recipe.particles.find(p => p.name === 'ParticleA')).consumedSlotConnections['slotA'].targetSlot);
        recipe.normalize();
    ...
    `);
        ...
        const recipe = manifest.recipes[0];
        assert.lengthOf(recipe.handles, 2);
        assert.equal(checkDefined(recipe.particles.find(p => p.name === 'ParticleA')).connections['slotA'].handle, checkDefined(recipe.particles.find(p => p.name === 'ParticleB')).connections['slotB2'].handle);
        const options = { errors: new Map(), details: '', showUnresolved: true };
        recipe.normalize(options);
  ...
  modality dom`;
    ...
    describe('trust claims and checks', () => {
      ...
      `);
            ...
            const claim1 = particle.trustClaims.find(claim => claim.handle.name === 'output1');
            assert.isNotNull(claim1);
            assert.equal(claim1.claims[0].isNot, true);
            assert.equal(claim1.claims[0].tag, 'property1');
        });
