...
describe('storage-proxy', () => {
    ...
    it('notifies for updates to initially empty handles', async () => {
        ...
        await engine.verify('InitializeProxy:foo', 'SynchronizeProxy:foo', 'InitializeProxy:bar', 'SynchronizeProxy:bar', 'onHandleSync:P1:foo:(null)', 'onHandleSync:P1:bar:[]');
    ...
    it('handles dropped updates on a Collection with delayed resync', async () => {
        ...
        await engine.verify('onHandleSync:P1:bar:[v1|v2|v3|v4|v5]', 'onHandleUpdate:P1:bar:+[v6]', 'onHandleUpdate:P1:bar:-[v1]');
    });
    it('handles misordered updates on a Collection', async () => {
        ...
        await engine.verify('SynchronizeProxy:bar', 'onHandleUpdate:P1:bar:+[v1]', 'onHandleDesync:P1:bar', 'onHandleUpdate:P1:bar:+[v2]', 'onHandleUpdate:P1:bar:+[v3]', 'onHandleUpdate:P1:bar:+[v4]', 'onHandleUpdate:P1:bar:+[v5]');
    });
    ...
    it('multiple particles observing one proxy', async () => {
        ...
        engine.sendSync(barStore);
        await engine.verify('InitializeProxy:bar', 'SynchronizeProxy:bar', 'onHandleSync:P1:bar:[]', 'onHandleSync:P2:bar:[]');
        // Drop event 2; desync is triggered by v3.
        barStore.store('i1', engine.newEntity('v1'));
        barStore.store('i2', engine.newEntity('v2'), { sendEvent: false });
        barStore.store('i3', engine.newEntity('v3'));
