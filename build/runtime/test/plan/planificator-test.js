...
describe('remote planificator', () => {
    ...
    async function verifyReplanning(producePlanificator, expectedSuggestions, expectedDescriptions = []) {
        ...
        for (const description of expectedDescriptions) {
            assert.lengthOf(producePlanificator.producer.result.suggestions.filter(s => s.descriptionText.includes(description)), 1, `Suggestion '${description}' is not found.`);
        }
    }
    ...
    it(`merges remotely produced suggestions`, async () => {
        ...
        const productsPlanificator = await Planificator.create(await createArc({ manifestFilename: productsManifestFilename }, storageKey), { userid, debug: false });
