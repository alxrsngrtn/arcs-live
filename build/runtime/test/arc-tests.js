...
describe('Arc', () => {
    ...
    it('optional handles aren\'t required to resolve', async () => {
        const { arc, recipe, thingClass, aStore, bStore, cStore, dStore } = await setupWithOptional(false, false);
        // NOTE: handleFor using incompatible types
        // tslint:disable-next-line: no-any
        await handleFor(aStore).set(new thingClass({ value: 'from_a' }));
        // tslint:disable-next-line: no-any
        await handleFor(cStore).set(new thingClass({ value: 'from_c' }));
    ...
    it('optional provided handles aren\'t required to resolve', async () => {
        const { arc, recipe, thingClass, aStore, bStore, cStore, dStore } = await setupWithOptional(true, false);
        // tslint:disable-next-line: no-any
        await handleFor(aStore).set(new thingClass({ value: 'from_a' }));
        // tslint:disable-next-line: no-any
        await handleFor(cStore).set(new thingClass({ value: 'from_c' }));
    ...
    it('optional provided handles resolve when provided', async () => {
        const { arc, recipe, thingClass, aStore, bStore, cStore, dStore } = await setupWithOptional(true, true);
        // tslint:disable-next-line: no-any
        await handleFor(aStore).set(new thingClass({ value: 'from_a' }));
        // tslint:disable-next-line: no-any
        await handleFor(cStore).set(new thingClass({ value: 'from_c' }));
    ...
    it('serialization roundtrip preserves data for volatile stores', async () => {
      ...
      `
        ...
        // tslint:disable-next-line: no-any
        await handleFor(varStore).set(new dataClass({ value: 'v1' }));
...
//# sourceMappingURL=arc-tests.js.map