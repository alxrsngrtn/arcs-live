...
${particleStr1}
  ...
  modality dom`;
    ...
    it('can parse a manifest containing an inline schema', async () => {
      ...
      particle Fooer
        foo: reads Foo {value}`);
        const verify = (manifest) => verifyPrimitiveType(manifest.schemas.Foo.fields.value, 'Text');
        verify(manifest);
        verify(await Manifest.parse(manifest.toString(), {}));
    });
    ...
    `);
        ...
        const recipe = manifest.recipes[0];
        assert.lengthOf(recipe.slots, 2);
        assert.strictEqual(checkDefined(recipe.particles.find(p => p.name === 'ParticleB')).getSlotConnectionByName('slotB1').providedSlots['slotB2'], checkDefined(recipe.particles.find(p => p.name === 'ParticleA')).getSlotConnectionByName('slotA').targetSlot);
        recipe.normalize();
        assert.isTrue(recipe.isResolved());
    });
    ...
    `);
        ...
        const recipe = manifest.recipes[0];
        assert.lengthOf(recipe.handles, 2);
        assert.strictEqual(checkDefined(recipe.particles.find(p => p.name === 'ParticleA')).connections['slotA'].handle, checkDefined(recipe.particles.find(p => p.name === 'ParticleB')).connections['slotB2'].handle);
        const options = { errors: new Map(), details: '', showUnresolved: true };
        assert.isTrue(recipe.normalize(options), 'normalizes');
...
Expected a verb (e.g. &Verb) or an uppercase identifier (e.g. Foo) but "?" found.
  ...
  store ClairesWishlist of [Product] #wishlist in 'wishlist.json'
    description \`Claire's wishlist\``, { loader });
        ...
        verify(manifest);
        assert.strictEqual(manifest.stores[0].toManifestString(), (await Manifest.parse(manifest.stores[0].toManifestString(), { loader })).toString());
...
`, {});
  ...
  modality dom`);
    ...
    `);
        ...
        assert.strictEqual(manifest.stores[0].toManifestString(), (await Manifest.parse(manifest.stores[0].toManifestString())).toString());
    });
  ...
  modality dom`;
    ...
    describe('trust claims and checks', () => {
      ...
      `);
            ...
            assert.strictEqual(claim1.claims[0].tag, 'property1');
            const claim2 = particle.trustClaims.find(claim => claim.handle.name === 'output2');
            assert.isNotNull(claim2);
            assert.strictEqual(claim2.claims[0].tag, 'property2');
        });
      ...
      `);
            ...
            assert.sameMembers(claim.claims.map(claim => claim.tag), ['property1', 'property2']);
        });
      ...
      `);
            ...
            const claim1 = particle.trustClaims.find(claim => claim.handle.name === 'output1');
            assert.isNotNull(claim1);
            assert.strictEqual(claim1.claims[0].isNot, true);
            assert.strictEqual(claim1.claims[0].tag, 'property1');
        });
      ...
      `);
            ...
            assert.isNotNull(claim);
            assert.sameMembers(claim.claims.map(claim => claim.parentHandle.name), ['input1', 'input2']);
        });
      ...
      `);
            ...
            const tagClaims = claim.claims.filter(claim => claim.type === ClaimType.IsTag);
            ...
            const derivesClaims = claim.claims.filter(claim => claim.type === ClaimType.DerivesFrom);
...
resource NobIdJson
  ...
  [{"nobId": "12345"}]
    ...
    it('works with old storage stack', async () => {
        ...
        assert.isTrue(store.storageKey.startsWith('volatile://'));
        const schema = store.type.getEntitySchema();
        assert.sameMembers(schema.names, ['NobIdStore']);
    });
});
