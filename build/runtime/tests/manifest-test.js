...
${particleStr1}
  ...
  modality dom`;
    ...
    `);
        ...
        const recipe = manifest.recipes[0];
        assert.lengthOf(recipe.slots, 2);
        assert.strictEqual(checkDefined(recipe.particles.find(p => p.name === 'ParticleB')).consumedSlotConnections['slotB1'].providedSlots['slotB2'], checkDefined(recipe.particles.find(p => p.name === 'ParticleA')).consumedSlotConnections['slotA'].targetSlot);
        recipe.normalize();
