...
Expected a verb (e.g. &Verb) or an uppercase identifier (e.g. Foo) but "?" found.
  ...
  store ClairesWishlist of [Product] #wishlist in 'wishlist.json'
    description \`Claire's wishlist\``, { loader });
        ...
        verify(manifest);
        assert.strictEqual(manifest.stores[0].toString([]), (await Manifest.parse(manifest.stores[0].toString([]), { loader })).toString());
...
`, {});
  ...
  modality dom`);
    ...
    `);
        ...
        assert.strictEqual(manifest.stores[0].toString([]), (await Manifest.parse(manifest.stores[0].toString([]))).toString());
    });
