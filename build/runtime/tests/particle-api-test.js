...
describe('particle-api', () => {
    it('StorageProxy integration test', async () => {
      ...
      `
        ...
        await inspector.verify('update:{"data":{"value":"v1"},"oldData":null}', 'desync', 'sync:{"value":"v3"}');
    ...
    it('particles can indicate that they are busy in setHandles', async () => {
      ...
      `
        ...
        recipe.normalize();
        await arc.instantiate(recipe);
        await inStore.set({ id: '1', rawData: {} }, 'a');
        await arc.idle;
  ...
  B as particle1
    detail: consume slot1`, 'Particle B should consume the detail slot provided by particle A');
    }));
});
