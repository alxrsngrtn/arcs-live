...
describe('recipe', () => {
  ...
  Specific as particle1
    ...
    `)).recipes[0];
        assert.isTrue(recipe.normalize());
        assert.isFalse(recipe.isResolved());
        assert.include(recipe.toString({ showUnresolved: true }), 'unresolved particle: unfulfilled slot connections');
    });
    ...
    `)).recipes[0];
        assert.isTrue(recipe.normalize());
        assert.isFalse(recipe.isResolved());
        assert.include(recipe.toString({ showUnresolved: true }), 'unresolved particle: unresolved connections');
    });
