...
describe('Arc new storage', () => {
    it('preserves data when round-tripping through serialization', Flags.withNewStorageStack(async () => {
      ...
      `
        ...
        const varHandle = await handleNGFor('crdt-key', varStorageProxy, arc.idGeneratorForTesting, null, true, true, 'varHandle');
        const colStorageProxy = new StorageProxyNG('id-2', await colStore.activate(), dataClass.type.collectionOf());
        const colHandle = await handleNGFor('crdt-key-2', colStorageProxy, arc.idGeneratorForTesting, null, true, true, 'colHandle');
        const refVarStorageProxy = new StorageProxyNG('id-3', await refVarStore.activate(), new SingletonType(dataClass.type));
        const refVarHandle = await handleNGFor('crdt-key-3', refVarStorageProxy, arc.idGeneratorForTesting, null, true, true, 'refVarHandle');
        ...
        const varHandle2 = await handleNGFor('crdt-key', varStorageProxy2, arc2.idGeneratorForTesting, null, true, true, 'varHandle');
        ...
        const colHandle2 = await handleNGFor('crdt-key-2', colStorageProxy2, arc2.idGeneratorForTesting, null, true, true, 'colHandle');
        ...
        const refVarHandle2 = await handleNGFor('crdt-key-3', refVarStorageProxy2, arc2.idGeneratorForTesting, null, true, true, 'refVarHandle');
...
['volatile://', 'pouchdb://memory/user-test/'].forEach((storageKeyPrefix) => {
    ...
    `, { loader, fileName: process.cwd() + '/input.manifest' });
        ...
        it('required provided handles must resolve with dependencies', async () => await assertThrowsAsync(async () => {
            const loader = new Loader();
            const manifest = await Manifest.parse(`
        schema Thing
          Text value
    ...
    `, { loader, fileName: '' });
      ...
      `,
            ...
            assert.deepEqual(['A', 'B'], serialization.particles.map(p => p.name), 'Spec of a particle referenced in an immediate mode should be serialized');
            assert.deepEqual(['HostedInterface'], serialization.interfaces.map(s => s.name), 'Hosted connection interface should be serialized');
