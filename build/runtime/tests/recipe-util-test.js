...
describe('recipe-util', () => {
    it('can produce a shape match to a simple recipe', async () => {
      ...
      recipe Recipe
        ...
        assert.strictEqual(results[0].match['A'].name, 'A');
        assert.strictEqual(results[0].match['B'].name, 'B');
        assert.strictEqual(results[0].match['v'].localName, 'handle1');
    });
    ...
    it('can produce multiple partial shape matches to a simple recipe', async () => {
      ...
      recipe Recipe
        ...
        assert.strictEqual(results[0].match['A'].name, 'A');
        assert.strictEqual(results[0].match['B'].name, 'B');
        assert.strictEqual(results[0].match['C'].name, 'C');
        assert.strictEqual(results[0].match['v'].localName, 'handle1');
        assert.strictEqual(results[1].score, -1);
        assert.strictEqual(results[1].match['A'].name, 'A');
        assert.strictEqual(results[1].match['B'].name, 'B');
        assert.strictEqual(results[1].match['C'].name, 'C');
        assert.strictEqual(results[1].match['v'].localName, 'handle2');
    });
    it('can match a free handle', async () => {
      ...
      recipe Recipe
        ...
        assert.strictEqual(results[0].match['v'].localName, 'h1');
    });
    it('can match dangling handle connections', async () => {
      ...
      recipe Recipe
        ...
        assert.lengthOf(results, 1);
        assert.strictEqual(results[0].score, -1);
        assert.strictEqual(results[0].match['h'].localName, 'h1');
        assert.strictEqual(results[0].match['A:a'].name, 'a');
        assert.strictEqual(results[0].match['B:b'].name, 'b');
    });
...
//# sourceMappingURL=recipe-util-test.js.map