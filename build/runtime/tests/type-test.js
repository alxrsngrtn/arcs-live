    ...
    describe('integration', () => {
      ...
      recipe MatchExisting
        ...
        it('a subtype matches to a supertype that wants to be read', async () => {
            ...
            assert.strictEqual(recipe.handles[0].type.getContainedType().canReadSubset.entitySchema.name, 'Lego');
            assert.strictEqual(recipe.handles[0].type.getContainedType().canWriteSuperset.entitySchema.name, 'Product');
        });
        it('a subtype matches to a supertype that wants to be read when a handle exists', async () => {
            ...
            assert.strictEqual(recipe.handles[0].type.getContainedType().entitySchema.name, 'Product');
        });
        it('a subtype matches to a supertype that wants to be read when a handle exists', async () => {
            const manifest = await Manifest.parse(manifestText);
            const recipe = manifest.recipes[1];
            ...
            assert(recipe.isResolved());
            assert.lengthOf(recipe.handles, 1);
            assert.strictEqual(recipe.handles[0].type.getContainedType().entitySchema.name, 'Lego');
        });
    });
});
