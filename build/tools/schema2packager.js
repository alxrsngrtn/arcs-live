...
function generate(schemaName, schema) {
    ...
    const processValue = (name, type, typeChar, passByReference) => {
        ...
        api.push(`${ref1}${type}${ref2} ${name}() const { return ${name}_; }`, `void set_${name}(${ref1}${type}${ref2} value) { ${name}_ = value; ${name}_valid_ = true; }`, `void clear_${name}() { ${name}_ = ${type}(); ${name}_valid_ = false; }`, `bool has_${name}() const { return ${name}_valid_; }`, ``);
        decode.push(`} else if (name == "${name}") {`, `  decoder.validate("${typeChar}");`, `  decoder.decode(entity->${name}_);`, `  entity->${name}_valid_ = true;`);
        encode.push(`if (entity.has_${name}())`, `  encoder.encode("${name}:${typeChar}", entity.${name}());`);
        toString.push(`if (entity.has_${name}())`, `  printer.add("${name}: ", entity.${name}());`);
    };
    let fieldCount = 0;
