...
function generate(schemaName, schema) {
    ...
    const processValue = (name, type, typeChar, condition, initial) => {
        ...
        encode.push(`if (${condition})`, `  encoder.encodeValue("${name}:${typeChar}", ${name}, "|");`);
        decode.push(`} else if (name == "${name}") {`, `  decoder.validate("${typeChar}");`, `  decoder.decodeValue(obj.${name});`);
    };
    const processCollection = (name, type, typeChar) => {
        decl.push(`std::unordered_set<${type}> ${name};`);
        encode.push(`if (!${name}.empty())`, `  encoder.encodeCollection("${name}:C${typeChar}", ${name});`);
        decode.push(`} else if (name == "${name}") {`, `  decoder.validate("C${typeChar}");`, `  decoder.decodeCollection(obj.${name});`);
    };
    let fieldCount = 0;
