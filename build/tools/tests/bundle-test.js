...
import { bundle, bundleListing } from '../bundle.js';
...
describe('Bundle Tool', () => {
    ...
    it('accepts multiple entry points', async () => {
        assert.deepEqual(relativize(await bundleListing('src/tools/tests/test-data/a/a.arcs', 'src/tools/tests/test-data/b/b.arcs')), [
    ...
    it('bundles Products demo', async () => {
        ...
        assert.strictEqual(await zip.file('__bundle_entry.arcs').async('text'), `import 'Products/Products.recipes'\n`);
    });
});
