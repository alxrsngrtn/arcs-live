...
describe('Tracing', () => {
    ...
    function assertTimestampsIncreasing(...timestmaps) {
        for (let i = 1; i < timestmaps.length; i++) {
            ...
            assert.isAtLeast(timestmaps[i], timestmaps[i - 1], `Expected sequence of timestamps not increasing at position ${i}`);
        }
    }
    function assertFlowEnclosedInComplete(flowEvent, completeEvent) {
        if (flowEvent.ph === 's') {
            assert.strictEqual(flowEvent.ts, completeEvent.ts + completeEvent.dur, `Flow should start at the end of the first complete event`);
        }
        else {
            assert.strictEqual(flowEvent.ts, completeEvent.ts, `Flow should step and end at the beginning of the respective complete event`);
        }
    }
    ...
    it('traces a sync event', async () => {
        ...
        assertTimestampsIncreasing(beginMicros, event.ts, event.ts + event.dur, endMicros);
    });
    ...
    it('traces an asynchronous event', async () => {
        ...
        assertTimestampsIncreasing(beginMicros, completeEvents[0].ts, insideFirstSync, completeEvents[0].ts + completeEvents[0].dur, betweenFirstAndSecond, completeEvents[1].ts, insideSecondSync, completeEvents[1].ts + completeEvents[1].dur, betweenSecondAndThird, completeEvents[2].ts, insideThirdSync, completeEvents[2].ts + completeEvents[2].dur, endMicros);
    ...
    it('traces an asynchronous event with endWith', async () => {
        ...
        assertTimestampsIncreasing(beginMicros, completeEvents[0].ts, insideFirstSync, completeEvents[0].ts + completeEvents[0].dur, betweenFirstAndSecond, completeEvents[1].ts, completeEvents[1].ts + completeEvents[1].dur, endMicros);
        assertFlowEnclosedInComplete(flowStartEvent, completeEvents[0]);
    ...
    it('traces flow events', async () => {
        ...
        const flowStepEvent = events.find(e => e.ph === 't');
        const flowEndEvent = events.find(e => e.ph === 'f');
        assertTimestampsIncreasing(beginMicros, flowStartEvent.ts, betweenStartAndStepMicros, flowStepEvent.ts, betweenStepAndFinishMicros, flowEndEvent.ts, endMicros);
    });
});
