...
describe('ArcStoresFetcher', () => {
    ...
    it('allows fetching a list of arc stores', async () => {
      ...
      schema Foo
        ...
        await fooStore.set({ value: 'persistence is useful' });
        assert.isEmpty(DevtoolsForTests.channel.messages.filter(m => m.messageType === 'fetch-stores-result'));
        await DevtoolsForTests.channel.receive({
            arcId: arc.id.toString(),
            messageType: 'fetch-stores'
        });
        const results = DevtoolsForTests.channel.messages.filter(m => m.messageType === 'fetch-stores-result');
    ...
    it('sends updates on value changes', async () => {
      ...
      recipe
        ...
        assert.isEmpty(DevtoolsForTests.channel.messages.filter(m => m.messageType === 'store-value-changed'));
        await arc.idle;
        const results = DevtoolsForTests.channel.messages.filter(m => m.messageType === 'store-value-changed');
        assert.lengthOf(results, 1);
...
//# sourceMappingURL=arc-stores-fetcher-test.js.map