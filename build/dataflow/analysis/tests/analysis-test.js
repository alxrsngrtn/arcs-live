...
function assertFailures(result, expectedFailures) {
    ...
    const expectedFailuresWithoutPaths = new Set();
 ...
 */
function createChainOfEdges(...nodeIds) {
  ...
  EdgeExpression(A->B) + {}
    ...
    describe('removeSelfReference', () => {
        ...
        it('combines self-modifiers with parent modifiers pair-wise', () => {
            ...
            expression.removeSelfReference();
            assert.hasAllKeys(expression.unresolvedFlows, [parentEdge]);
            assert.isTrue(expression.unresolvedFlows.get(parentEdge).equals(new FlowModifierSet(addsTag('parent1'), addsTag('parent1', 'self1'), addsTag('parent1', 'self2'), addsTag('parent2'), addsTag('parent2', 'self1'), addsTag('parent2', 'self2'))));
        });
        it('combines self-modifiers with resolved flows pair-wise', () => {
            ...
            expression.removeSelfReference();
            assert.isEmpty(expression.unresolvedFlows);
            assert.isTrue(expression.resolvedFlows.equals(new FlowSet(addsTag('resolved1').toFlow(), addsTag('resolved1', 'self1').toFlow(), addsTag('resolved1', 'self2').toFlow(), addsTag('resolved2').toFlow(), addsTag('resolved2', 'self1').toFlow(), addsTag('resolved2', 'self2').toFlow())));
        });
    });
});
describe('Solver', () => {
    ...
    describe(`checks using the 'and' operator`, () => {
      ...
      `);
        ...
        it(`handles nesting of boolean conditions`, async () => {
            const validateCondition = async (checkCondition) => {
                const graph = await buildFlowGraph(`
          particle P1
            out Foo {} output
            claim output is trusted
    ...
    describe('cycles', () => {
      ...
      `);
        ...
        it(`supports breaking cycles using 'derives from' claims`, async () => {
            const runWithCheck = async (check) => {
                const graph = await buildFlowGraph(`
          particle P
            in Foo {} input1
            in Foo {} input2
