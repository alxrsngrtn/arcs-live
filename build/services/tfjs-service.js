 ...
 */
import { ResourceManager as rmgr } from './resource-manager.js';
 ...
 */
const loadGraphModel = async ({ modelUrl, options }) => {
 ...
 * @return {Reference} A reference to the in-memory model.
 */
const loadLayersModel = async ({ modelUrl, options }) => {
    const tf = await requireTf();
    log('Loading model...');
 ...
 */
const predict = async ({ model, inputs, config }) => {
    log('Referencing model and input...');
    const model_ = rmgr.deref(model);
...
const warmUp = async ({ model }) => {
    ...
    const result = await model_.predict(zeroInput, {});
    result.dispose();
    log('Model warm.');
};
/** Clean up resources */
 ...
 */
const normalize = async ({ input, range = [0, 255] }) => {
    log('Normalizing...');
 ...
 */
const resizeBilinear = async ({ images, size, alignCorners }) => {
    const tf = await requireTf();
 ...
 */
const getTopKClasses = async ({ input, y, yHat, labels, topK = 3 }) => {
    log('Getting top K classes...');
    const input_ = rmgr.deref(input || y || yHat);
...
//# sourceMappingURL=tfjs-service.js.map