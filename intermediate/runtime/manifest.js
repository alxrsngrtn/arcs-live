...
export class Manifest {
    ...
    createStore(type, name, id, tags, storageKey) {
        return __awaiter(this, void 0, void 0, function* () {
            assert(!type.hasVariableReference, `stores can't have variable references`);
            let store = yield this.storageProviderFactory.construct(id, type, storageKey || `in-memory://${this.id}`);
    ...
    *_findAll(manifestFinder) {
    ...
    findStoreByType(type, options = {}) {
        ...
        return stores.filter(s => !!Handle.effectiveType(type, [{ type: s.type, direction: s.type.isInterface ? 'host' : 'inout' }]));
    }
    ...
    static load(fileName, loader, options) {
        return __awaiter(this, void 0, void 0, function* () {
            options = options || {};
            let { registry, id } = options;
            registry = registry || {};
            if (registry && registry[fileName]) {
                return yield registry[fileName];
            }
            registry[fileName] = (() => __awaiter(this, void 0, void 0, function* () {
                let content = yield loader.loadResource(fileName);
                ...
                return yield Manifest.parse(content, {
                    id,
                    fileName,
                    loader,
                    registry,
...
${e.message}
  ...
  ${highlight}`;
            ...
            try {
                ...
                yield Promise.all(items.filter(item => item.kind == 'import').map((item) => __awaiter(this, void 0, void 0, function* () {
                    let path = loader.path(manifest.fileName);
                    let target = loader.join(path, item.path);
                    try {
                        manifest._imports.push(yield Manifest.load(target, loader, { registry }));
                    }
                    catch (e) {
                ...
                let processItems = (kind, f) => __awaiter(this, void 0, void 0, function* () {
                ...
                yield processItems('meta', meta => manifest.applyMeta(meta.items));
                // similarly, resources may be referenced from other parts of the manifest.
                yield processItems('resource', item => this._processResource(manifest, item));
                yield processItems('schema', item => this._processSchema(manifest, item));
                yield processItems('shape', item => this._processShape(manifest, item));
                yield processItems('particle', item => this._processParticle(manifest, item, loader));
                yield processItems('store', item => this._processStore(manifest, item, loader));
                yield processItems('recipe', item => this._processRecipe(manifest, item, loader));
            }
            catch (e) {
            ...
            visit(node, visitChildren) {
                ...
                switch (node.kind) {
                    ...
                    case 'reference-type': {
                        ...
                        if (!resolved) {
                            throw new ManifestError(node.location, `Could not resolve type reference '${node.name}'`);
                        }
                        if (resolved.schema) {
            ...
            for (let [particle, item] of items.byParticle) {
                for (let connectionItem of item.connections) {
                    ...
                    if (connectionItem.target && connectionItem.target.particle) {
                        ...
                        let particleSpecHash = yield digest(JSON.stringify(hostedParticleLiteral));
                        let id = `${manifest.generateID()}:${particleSpecHash}:${hostedParticle.name}`;
                        targetHandle = recipe.newHandle();
                        targetHandle.fate = 'copy';
                        let store = yield manifest.createStore(connection.type, null, id, []);
    ...
    static _processStore(manifest, item, loader) {
        return __awaiter(this, void 0, void 0, function* () {
            let name = item.name;
            let id = item.id;
            ...
            if (item.origin == 'file') {
                ...
                // TODO: json5?
                json = yield loader.loadResource(source);
            }
            ...
            if (!type.isCollection) {
                if (entities.length == 0) {
                    yield Manifest._createStore(manifest, type, name, id, tags, item);
                    return;
                }
            ...
            if (unitType.isEntity) {
                ...
                if (!item.id && !hasSerializedId) {
                    let entityHash = yield digest(JSON.stringify(entities.map(entity => entity.rawData)));
                    id = `${id}:${entityHash}`;
                }
            }
            let version = item.version || 0;
            let store = yield Manifest._createStore(manifest, type, name, id, tags, item);
    ...
    static _createStore(manifest, type, name, id, tags, item) {
        return __awaiter(this, void 0, void 0, function* () {
            let store = yield manifest.createStore(type, name, id, tags);
