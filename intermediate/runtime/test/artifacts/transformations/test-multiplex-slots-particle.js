        ...
        willReceiveProps({ foos }) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                for (let [index, foo] of foos.entries()) {
                    ...
                    let fooHandle = yield arc.createHandle(type.primitiveType(), 'foo' + index);
                    this._handleIds.add(foo.id);
                    let hostedSlotName = [...hostedParticle.slots.keys()][0];
                    let slotName = [...this.spec.slots.values()][0].name;
                    let slotId = yield arc.createSlot(this, slotName, hostedParticle.name, hostedSlotName);
                    let recipe = `
          schema Foo
            Text value
