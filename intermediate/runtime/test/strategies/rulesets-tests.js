...
describe('Rulesets', () => {
    ...
    const planAndComputeStats = (options) => __awaiter(this, void 0, void 0, function* () {
        let arc = StrategyTestHelper.createTestArc('test-plan-arc', options.context, 'mock');
        let planner = new Planner();
        planner.init(arc, options);
        let generations = [];
        yield planner.plan(Infinity, generations);
    ...
    it('respects ordering rules in a small example', () => __awaiter(this, void 0, void 0, function* () {
        const strategies = [InitPopulation, AssignFateA, AssignFateB];
        const context = yield Manifest.parse(`
      recipe
        ? 'id1' as handle1
        ? 'id2' as handle2`);
        const statsNoRules = yield planAndComputeStats({
            context,
            strategies
        });
        const statsLinear = yield planAndComputeStats({
            context,
            strategies,
    ...
    it('respects ordering rules in a big example', () => __awaiter(this, void 0, void 0, function* () {
        const strategies = [
      ...
      recipe
        ...
        const statsPhased = yield planAndComputeStats({
            context,
            strategies,
            ruleset: new Ruleset.Builder()
