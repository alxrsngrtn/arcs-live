...
describe('Multiplexer', function () {
    it('Processes multiple inputs', () => __awaiter(this, void 0, void 0, function* () {
    ...
    `, { loader, fileName: './manifest.manifest' });
        ...
        let barStore = yield arc.createStore(barType.collectionOf(), null, 'test:1');
        recipe.handles[0].mapToStorage(barStore);
        assert(recipe.normalize());
        assert(recipe.isResolved());
        yield arc.instantiate(recipe);
        yield arc.idle;
        yield barStore.store({ id: 'a', rawData: { value: 'one' } }, ['key1']);
        yield barStore.store({ id: 'b', rawData: { value: 'two' } }, ['key2']);
        yield barStore.store({ id: 'c', rawData: { value: 'three' } }, ['key3']);
        yield arc.idle;
        assert.equal(slotsCreated, 3);
    }));
    it('renders polymorphic multiplexed slots', function () {
        return __awaiter(this, void 0, void 0, function* () {
            let helper = yield TestHelper.create({
            ...
            yield postsStore.store({ id: '1', rawData: { message: 'x', renderRecipe: recipeOne, renderParticleSpec: showOneSpec } }, ['key1']);
            yield postsStore.store({ id: '2', rawData: { message: 'y', renderRecipe: recipeTwo, renderParticleSpec: showTwoSpec } }, ['key2']);
            yield postsStore.store({ id: '3', rawData: { message: 'z', renderRecipe: recipeOne, renderParticleSpec: showOneSpec } }, ['key3']);
            ...
            yield helper.acceptSuggestion({ particles: ['PostMuxer', 'List'] });
            ...
            helper.slotComposer
                ...
                .expectRenderSlot('PostMuxer', 'item', { contentTypes: ['templateName', 'model'] });
            yield postsStore.store({ id: '4', rawData: { message: 'w', renderRecipe: recipeOne, renderParticleSpec: showOneSpec } }, ['key1']);
            yield helper.idle();
