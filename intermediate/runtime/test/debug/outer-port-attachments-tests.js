...
describe('OuterPortAttachment', function () {
    ...
    it('produces dataflow messages on devtools channel', () => __awaiter(this, void 0, void 0, function* () {
        Random.seedForTests();
        ...
        const fooStore = yield arc.createStore(Foo.type, undefined, 'fooStore');
        const recipe = arc._context.recipes[0];
        recipe.handles[0].mapToStorage(fooStore);
        recipe.normalize();
        yield arc.instantiate(recipe);
        let instantiateParticleCall = DevtoolsForTests.channel.messages.find(m => m.messageType === 'InstantiateParticle').messageBody;
        ...
        yield util.assertSingletonWillChangeTo(arc, fooStore, 'value', 'FooBar');
        let dateflowSetCall = DevtoolsForTests.channel.messages.find(m => m.messageType === 'dataflow' &&
