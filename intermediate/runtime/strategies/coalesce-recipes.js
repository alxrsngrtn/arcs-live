                ...
                onSlot(recipe, slot) {
                    ...
                    for (let { slotConn, matchingHandles } of index.findConsumeSlotConnectionMatch(slot)) {
                        ...
                        results.push((recipe, slot) => {
                            ...
                            let otherToHandle = index.findCoalescableHandles(recipe, slotConn.recipe, new Set(slot.handleConnections.map(hc => hc.handle).concat(matchingHandles.map(({ handle, matchingConn }) => matchingConn.handle))));
                    ...
                    for (let otherHandle of index.findHandleMatch(handle, index.coalescableFates)) {
                        ...
                        if (otherHandle.type.hasVariable) {
                            ...
                            if (resolved.isCollection)
                                resolved = resolved.collectionType;
