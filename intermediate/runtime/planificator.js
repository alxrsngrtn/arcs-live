...
export class Planificator {
    ...
    _onPlanInstantiated(plan) {
        ...
        assert(this._current.plans.some(currentPlan => currentPlan.plan.toString() == planString), `The instantiated plan (${plan.toString()}) doesn't appear in the current plans.`);
    ...
    _doNextPlans({ strategies, strategyArgs, timeout = defaultTimeoutMs }) {
        return __awaiter(this, void 0, void 0, function* () {
            this._planner = new Planner();
            this._planner.init(this._arc, { strategies, strategyArgs });
            this._next.plans = yield this._planner.suggest(timeout, this._next.generations, this._speculator);
            this._planner = null;
        });
    }
    ...
    _shouldRequestContextualPlanning() {
        ...
        if (this._suggestFilter.showAll || this._suggestFilter.search)
            return false;
        return this._isArcPopulated();
    }
    _isArcPopulated() {
        if (this._arc.recipes.length == 0)
            return false;
