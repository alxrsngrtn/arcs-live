 ...
 */
class K18sDeployment {
    constructor(k8sApi, v1Deployment, v1Service, ingress) {
        this.k8sApi = k8sApi;
    ...
    async node() {
        const { body: v1Pod } = await this.k8sApi.readNamespacedPod(this.v1Deployment.metadata.name, K18S_NAMESPACE, undefined, false, false);
        return Promise.resolve(v1Pod.spec.nodeName);
    }
}
export class K18sContainerManager {
    constructor() {
    ...
    async deploy(fingerprint, rewrappedKey, encryptedDisk) {
    ...
    async requestUpdateIngress(fingerprint, service) {
        const { body: ingress } = await this.k8sBetaApi.readNamespacedIngress(this.k8sName, K18S_NAMESPACE, undefined, false, false);
        ...
        try {
            return await this.k8sBetaApi.patchNamespacedIngress(this.k8sName, K18S_NAMESPACE, newIngress, undefined);
        }
        finally {
            this.k8sBetaApi['defaultHeaders'] = oldHeaders;
        }
    }
    async requestCreateService(fingerprint, deployment) {
    ...
    async requestCreateDeployment(encryptedDisk, fingerprint, rewrappedKey, node) {
    ...
    createContainer(fingerprint, volumeName) {
     ...
     */
    createGCEVolume(volumeName, encryptedDisk) {
     ...
     */
    async requestNewPersistentVolume(encryptedDisk) {
    ...
    makePersistentVolumeSpec(encryptedDisk) {
    ...
    async find(fingerprint) {
        const { body: v1Deployment } = await this.k8sBetaApi.listNamespacedDeployment(K18S_NAMESPACE, undefined, undefined, undefined, true, undefined);
        const { body: ingress } = await this.k8sBetaApi.readNamespacedIngress(this.k8sName, K18S_NAMESPACE, undefined, false, false);
        const deploymentList = v1Deployment.items;
        if (deploymentList.length > 0) {
            const { body: v1Service } = await this.k8sApi.listNamespacedService(K18S_NAMESPACE, undefined, undefined, "metadata.name=svc-" + fingerprint, true, undefined);
