...
export class K18sContainerManager {
    ...
    async find(fingerprint) {
        const { body: v1Deployment } = await this.k8sBetaApi.listNamespacedDeployment(K18S_NAMESPACE, true // includeUnitialized
        );
        const { body: ingress } = await this.k8sBetaApi.readNamespacedIngress(this.k8sName, K18S_NAMESPACE, undefined, false, false);
        const deploymentList = v1Deployment.items;
        if (deploymentList.length > 0) {
            const { body: v1Service } = await this.k8sApi.listNamespacedService(K18S_NAMESPACE, true, // includeUnitialized
            undefined, // pretty
            undefined, // _continue
