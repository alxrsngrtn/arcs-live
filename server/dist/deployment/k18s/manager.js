import { Core_v1Api, KubeConfig, V1Container, V1ContainerPort, V1EnvVar, V1GCEPersistentDiskVolumeSource, V1ObjectMeta, V1PersistentVolume, V1PersistentVolumeSpec, V1Pod, V1PodSpec, V1Service, V1ServicePort, V1ServiceSpec, V1Volume, V1VolumeMount } from "@kubernetes/client-node";
...
export class K18sContainerManager {
    ...
    async deploy(fingerprint, encryptedDisk) {
    ...
    async requestCreateService(fingerprint, pod) {
    ...
    async requestCreatePod(encryptedDisk, fingerprint) {
    ...
    async find(fingerprint) {
        ...
        const { response, body } = await k8sApi.listNamespacedPod('default', undefined, undefined, undefined, true, undefined);
        const podList = body.items;
        if (podList.length > 0) {
            const { response, body } = await k8sApi.listNamespacedService('default', undefined, undefined, "metadata.name=svc-" + fingerprint, true, undefined);
